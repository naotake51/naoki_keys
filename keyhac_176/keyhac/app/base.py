...
class SubNomalKeymap(Keymap):
    def e(self):set_ime_status(False)
    def n(self):set_ime_status(True)
...
class App():
    ...
    def configure(self, keymap):
        if self.exe_name:
            keymap = keymap.defineWindowKeymap(exe_name=self.exe_name, class_name=self.class_name)
        else:
            keymap = keymap.defineWindowKeymap()
    
        keymap["D-29"] = self.down_nomal_mode_key
        keymap["U-29"] = self.up_nomal_mode_key
        keymap["D-28"] = self.down_app_mode_key
        keymap["U-28"] = self.up_app_mode_key
