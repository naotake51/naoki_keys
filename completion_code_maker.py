...
def make_comletion_code(completion_text):
	...
	completion_text_ahk_string = relpace_ahk_escape_char(completion_text)
	completion_text_ahk_string = delete_cursor_symbol(completion_text_ahk_string)
	...
	if "@" in completion_text:
		...
		completion_code = completion_code.replace("[::cursor_up_count::]", str(text_after_symbol.count("\n")))
		completion_code = completion_code.replace("[::cursor_left_count::]", str(len(text_bettween_symbol_lf)))
	else:
		put_to_ahk_code_tmplate("[::move_cursor_comment_out::]", ";;")
...
with open(input_completion_def_file,'r', encoding='utf-8') as f:
	...
	while line:
		if line == ">>>\n":
			...
			completion_code = make_comletion_code(part_replace_in_completion_code)
			part_replace_in_completion_code = ""

			output_ahk_code += completion_code
		else:
